...
[opencode](https://github.com/anomalyco/opencode)는 터미널 기반 AI 코딩 에이전트입니다. KIMS 프록시를 통해 동일하게 사용할 수 있습니다.
...
이 레포의 `opencode.json`을 글로벌 설정 경로에 복사합니다:
...
`opencode.json`은 `kims` 프로바이더를 `@ai-sdk/openai-compatible`로 등록하며, 프록시(`http://localhost:4000/api/v1`)를 통해 모든 모델에 접근합니다.
...
# 단일 질문 실행
opencode run --model kims/claude-sonnet-4-6 "질문 내용"
...
```
kims/claude-haiku
kims/claude-opus-4-6
kims/claude-sonnet-4-6
kims/gpt-5.4-high
kims/gpt-5.4-low
kims/gpt-5.4-medium
kims/gpt-5.4-none
kims/gpt-5.4-xhigh
kims/perplexity-sonar
kims/perplexity-sonar-pro
kims/perplexity-sonar-reasoning
```

> **참고**: opencode의 `apiKey`에 `sk-dummy`를 사용하는 이유는 Roo Code와 동일합니다. 실제 인증은 프록시가 처리합니다.
